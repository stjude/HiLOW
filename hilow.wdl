...
### ---------------------------------------- ###
    ...
    #3. Create FASTA index and chrom sizes files
    ...
    #4. Create promoters file from gtf as chr,start,end,TranscriptID|GeneID
...
### ---------------------------------------- ###
        ...
        if ( defined(promoters) || defined(geneAnnotation) ){
            call twoDloops as active_twoDloops {
                input :
                    count_fastqs=step2hicpro.count_fastqs,
                    FDR=FDR,
                    hicpro_out=hicpro_output,
            ...
            call qcreport as active_qcreport {
                input:
                    sampleid=sampleid,
                    genomename=genomename,
                    activeregion=true,
...
task qcreport {
    input {
        ...
        String genomename
        String? sampleid
        String sampleid_m = if activeregion then 'active_' + if defined(sampleid) then sampleid + '.' + genomename + '_' else "" else if defined(sampleid) then sampleid + '.' + genomename + '_' else ""
